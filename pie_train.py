...
class AE_full(object):
    def __init__(self, enc_dim_list, dec_dim_list, num_views=3, learning_rate=1e-3, batch_size=1428, reg=None,
                 model_path=None, restore_path=None, logs_path='./Net_Models_logs/PIE_Pre_logs_multiview'):
...
def load_data(file_name):
    ...
    x1, x2, x3, gt = dataset['x1'], dataset['x2'], dataset['x3'], dataset['gt']
    gt = gt.flatten()
...
if __name__ == '__main__':
    x0, x1, x2, gt = load_data('./datasets/PIE.mat')
    x0 = minmax_scale(x0)
    x1 = minmax_scale(x1)
    x2 = minmax_scale(x2)
    x = {}
    ...
    x['2'] = x2
    num_views = 3
    enc_dim_list = [[900, 1024, 128], [512, 1024, 128], [2560, 1024, 128]]
    dec_dim_list = [[1024, 900], [1024, 512], [1024, 2560]]
    batch_size = 1428
    model_path = './Net_Models_logs/Model_multiview/PIE_model.ckpt'

    AE = AE_full(enc_dim_list=enc_dim_list, dec_dim_list=dec_dim_list, num_views=3, learning_rate=1e-3,
